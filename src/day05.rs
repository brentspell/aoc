...
pub fn solve() {
    ...
    let result = input
        ...
        .filter(|l| l.chars().zip(l.chars().skip(1)).any(|(x, y)| x == y))
    ...
    let result = input
        .iter()
        .filter(|l| {
            let pairs: Vec<(char, char)> = l.chars().zip(l.chars().skip(1)).collect();
            pairs
                .iter()
                .enumerate()
                .any(|(i, p)| pairs.iter().skip(i + 2).any(|q| q == p))
        })
        .filter(|l| l.chars().zip(l.chars().skip(2)).any(|(x, y)| x == y))
        .count();
